...
static void stack_push(MatrixStack* stack, Matrix* value) {
    if (stack->size == stack->capacity) {
        ...
        if (new_items == NULL) {
            fprintf(stderr, "Не удалось расширить тестовый стек опз\n");
            exit(1);
        }
...
static Matrix* stack_pop(MatrixStack* stack) {
    if (stack->size == 0) {
        fprintf(stderr, "Ошибка: недостаточно операндов в формуле ОПЗ\n");
        exit(1);
    }
...
static Matrix* evaluate_rpn_expression(const char* expression,
    ...
    if (buffer == NULL) {
        fprintf(stderr, "Не удалось выделить память для выражения обратной польской записи\n");
        exit(1);
    }
    ...
    for (char* token = strtok(buffer, " \t\r\n"); token != NULL; token = strtok(NULL, " \t\r\n")) {
        ...
        if (is_number_token(token)) {
            if (field_info->type != INT_TYPE) {
                fprintf(stderr, "Поддерживаются только скалярные значения в тестах\n");
                free(buffer);
                stack_destroy(&stack);
                exit(1);
            }
        ...
        if (found == NULL) {
            fprintf(stderr, "Неизвестный токен в выражении обратной польской записи: %s\n", token);
            free(buffer);
            stack_destroy(&stack);
            exit(1);
        }
    ...
    if (stack.size != 1) {
        fprintf(stderr, "Неверная формула обратной польской записи: %d элементов осталось в стеке после вычисления\\n", stack.size);
        stack_destroy(&stack);
        exit(1);
    }
