...
static void stack_push(MatrixStack* stack, Matrix* value) {
    if (stack->size == stack->capacity) {
        ...
        if (new_items == NULL) {
            fprintf(stderr, "Failed to grow RPN test stack\n");
            exit(1);
        }
...
static Matrix* stack_pop(MatrixStack* stack) {
    if (stack->size == 0) {
        fprintf(stderr, "Invalid RPN formula: not enough operands\n");
        exit(1);
    }
...
static Matrix* evaluate_rpn_expression(const char* expression,
    ...
    if (buffer == NULL) {
        fprintf(stderr, "Failed to allocate memory for RPN expression\n");
        exit(1);
    }
    ...
    for (char* token = strtok(buffer, " \t\r\n"); token != NULL; token = strtok(NULL, " \t\r\n")) {
        ...
        if (is_number_token(token)) {
            if (field_info->type != INT_TYPE) {
                fprintf(stderr, "Numeric scalars are supported only in INT RPN tests\n");
                free(buffer);
                stack_destroy(&stack);
                exit(1);
            }
        ...
        if (found == NULL) {
            fprintf(stderr, "Unknown token in RPN expression: %s\n", token);
            free(buffer);
            stack_destroy(&stack);
            exit(1);
        }
    ...
    if (stack.size != 1) {
        fprintf(stderr, "Invalid RPN formula: %d items left in stack after evaluation\n", stack.size);
        stack_destroy(&stack);
        exit(1);
    }
